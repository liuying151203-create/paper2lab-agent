...
class LlmPaperCardExtractor:
    ...
    def extract(self, paper_id: str, chunks: list[Chunk]) -> PaperCard:
        ...
        try:
            ...
            card = _paper_card_from_llm_json(
                paper_id=paper_id,
                response=response,
                chunks=chunks,
            )
            return card.model_copy(
                update={
                    ...
                    "extraction_notes": [
                        "Generated by configured OpenAI-compatible chat model."
                    ],
                }
            )
        except (LlmGenerationError, ValueError, TypeError, json.JSONDecodeError) as exc:
            fallback_card = self.fallback.extract(paper_id=paper_id, chunks=chunks)
            return fallback_card.model_copy(
                update={
                    "extraction_notes": [
